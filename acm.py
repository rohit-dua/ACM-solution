...
class ACM(object):
	...
	def calculate_max_profit(self):
		...
		max_profit = self.execute_calculate_max_profit(day_index=0, cash=self.balance, current_machine=None)
		return max_profit

	def execute_calculate_max_profit(self, day_index, cash, current_machine):
		# Each day 2 options: Sell/Buy or Stay
		# Can skip days of non sale
		if day_index >= len(self.sale_days):
			days_left = self.number_of_days - self.sale_days[-1]
			if current_machine is not None:
				cash = cash + (days_left * current_machine.profit_per_day)
				cash = cash + current_machine.selling_price
			return cash
		day = self.sale_days[day_index]
		if day_index != 0 and current_machine is not None:
			previous_day = self.sale_days[day_index - 1]
			cash = cash + (day - previous_day - 1)*current_machine.profit_per_day
		# Each selling day 2 options: Sell/Buy(for each item) or Stay
		#stay
		if current_machine is None:
			stay_result = self.execute_calculate_max_profit(day_index+1, cash, current_machine)
		else:
			stay_result = self.execute_calculate_max_profit(day_index+1, cash + current_machine.profit_per_day, current_machine)
		#Sell and buy
		#sell
		if current_machine is not None:
		...
		for machine in self.machines_per_day[day]:
			if machine.cost_price <= cash:
				value = self.execute_calculate_max_profit(day_index+1, cash-machine.cost_price, machine)
